...
import { Send, Bot, User, Upload, FileText, Loader2, RefreshCw, Paperclip, Activity, TrendingUp, Shield, Zap } from 'lucide-react';
...
const RiskResultCard = ({ riskData }) => {
    ...
    return (
        <motion.div 
            ...
            className={`${colors.bg} ${colors.border} border rounded-2xl p-5 my-3 max-w-md`}
        >
...
const Chatbot = ({ onAssessmentComplete }) => {
    ...
    const [riskResults, setRiskResults] = useState([]); // track which message indices have risk cards
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            const newMsgIndex = messages.length + 1; // +1 for the user message we just added
            
            setMessages(prev => [...prev, { role: 'model', content: res.data.reply }]);
            
            if (res.data.complete && res.data.riskAssessment) {
                // Store risk result linked to this message index
                setRiskResults(prev => [...prev, { index: newMsgIndex, data: res.data.riskAssessment }]);
                
                // Still call onAssessmentComplete for any parent state needs
                if (onAssessmentComplete) {
                    onAssessmentComplete(res.data.riskAssessment);
                }
            }
        } catch (error) {
            setMessages(prev => [...prev, { role: 'model', content: "I'm sorry, I'm having trouble connecting to the server. Please try again." }]);
        } finally {
            setLoading(false);
        }
    };
                        ...
                        <div key={idx}>
                            ...
                            >
                                ...
                                <div className={`max-w-[75%] ${
                                    msg.role === 'user' 
                                        ? 'bg-slate-100 text-slate-900 rounded-2xl px-5 py-3' 
                                        : 'bg-transparent text-slate-900 py-1'
                                }`}>
                            ...
                            {/* Inline risk result card right after the message */}
                            {riskForThis && (
                                <div className="max-w-4xl mx-auto pl-12 mt-3">
                                    <RiskResultCard riskData={riskForThis.data} />
                                    {/* Deeper insights button */}
                                    <motion.button
                                        initial={{ opacity: 0, y: 5 }}
                                        animate={{ opacity: 1, y: 0 }}
                                        transition={{ delay: 0.5 }}
                                        onClick={handleDeeperInsights}
                                        className="flex items-center gap-2 mt-3 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-sm font-medium transition-colors shadow-sm"
                                    >
            ...
            <div className="p-4 bg-white shrink-0">
                <form onSubmit={handleSend} className="relative flex items-center gap-2 max-w-4xl mx-auto">
                    <button 
                        ...
                        className="flex items-center justify-center p-3 text-slate-500 hover:text-slate-900 bg-white hover:bg-slate-100 rounded-xl transition-colors shrink-0 outline-none"
                        title="New Chat"
                    >
                    ...
                    <div className="relative flex-1">
                        <input
                            ...
                            className="w-full bg-slate-100 hover:bg-slate-200 focus:bg-white border border-transparent focus:border-slate-300 text-[15px] text-slate-900 rounded-2xl pl-12 pr-4 py-3.5 outline-none transition-colors"
                            disabled={loading || uploading}
                        />
                        
                        <div className="absolute left-1.5 top-1/2 -translate-y-1/2 rounded-full">
                            <label className="cursor-pointer flex items-center justify-center p-2 text-slate-500 hover:text-slate-900 transition-colors">
                    ...
                    <button 
                        ...
                        className="p-3.5 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-xl transition-colors shrink-0 outline-none"
                    >
